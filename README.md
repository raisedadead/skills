...
| [dossier](dossier/)                 | Stack-neutral, file-based, phase-driven workflow for any multi-phase initiative — frontend, backend, CLI, library, mobile, data, infra, ML. Three living docs in `.dossier/` (audit ledger, phase plan, covenant) + committed `.changeset/`, plus 8 opt-in `lenses/` for stack-specific gates and footguns. Composes with `obra/superpowers` (brainstorm/plan/dispatch/review) — dossier owns the durable state. Replaces heavyweight stage plugins (dp-cto, dp-cab) with plain markdown + native Tasks API. | Bash 4+ (init script) |
